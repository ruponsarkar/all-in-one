...
class JournalController extends Controller
{
    ...
    function journal(Request $request, $slug)
    {

        $journal = journal::where('slug', $slug)->first();

        $v = $request->query('volume') ?? 'all';
        $i = $request->query('issue') ?? 'all';
        ...
        if ($v === 'all') {
            ...
            $archive = DB::table('article')
                ->join('volume', 'volume.id', 'article.v_id')
                ->join('issues', 'issues.id', 'article.i_id')

                ->where('article.j_id', $journal->j_id)
                ->where('article.status', 1)
                ->select('article.*', 'volume.name as v_name', 'volume.year as v_year', 'issues.name as i_name')
                ->get();
            ...
            $v_id = volumes::where('short', $v)->where('j_id', $journal->j_id)->first();
            if ($v_id) {
                $v_id = $v_id->id;
            }
            // return $v_id;
            ...
            if ($i === 'all') {
                ...
                $i_id = issue::where('short', $i)->where('v_id', $v_id)->where('j_id', $journal->j_id)->first();
        ...
        $volumes = volumes::where('j_id', $journal->j_id)->get();
        $issues = issue::where('j_id', $journal->j_id)->get();

        // $archive = articles::where('');
        // return $volume . ' - '. $issue;
        // $allJournals = journal::get()->where('active', 1);
        // return $allJournals;


        return view('usa_archive', [
            'archive' => $archive,
            'journal' => $journal,
            'type' => 'archive',
            'volumes' => $volumes,
    ...
    function journalParams($slug, $params)
    {
        $allJournals = journal::get()->where('active', 1);

        $journal = journal::where('slug', $slug)->first();

        $editors = editor_data::where('j_id', $journal->j_id)
            ->where('active', 1)
            ->get()
            ->groupBy('type');
        ...
        try {
            ...
            if ($params === 'guidlines') {
                $content = DB::table('pages')->where('type', 'author')->orderBy('id', 'desc')->first()->data;
            }
            if ($params === 'OpenAccessPolicy') {
                $content = DB::table('pages')->where('type', 'OpenAccessPolicy')->orderBy('id', 'desc')->first()->data;
            }
            if ($params === 'journalPolicy') {
                $content = DB::table('pages')->where('type', 'journalPolicy')->orderBy('id', 'desc')->first()->data;
            }
            if ($params === 'copyrightNotice') {
                $content = DB::table('pages')->where('type', 'copyrightNotice')->orderBy('id', 'desc')->first()->data;
            }
            if ($params === 'publicationEthics') {
                $content = DB::table('pages')->where('type', 'publicationEthics')->orderBy('id', 'desc')->first()->data;
            }
            if ($params === 'fee') {
                $content = DB::table('pages')->where('type', 'fee')->orderBy('id', 'desc')->first()->data;
            }
            if ($params === 'contact') {
                $content = DB::table('pages')->where('type', 'contact')->orderBy('id', 'desc')->first()->data;
            }

            $indexings = '';
            if ($params === 'indexings') {
                $indexings = indexings::select(['link', 'img'])
    ...
    function aboutUs()
    {
        $content = DB::table('pages')->where('type', 'about')->orderBy('id', 'desc')->first();

        // return $content;

        return view('usa_about', ['data' => $content]);
    }
